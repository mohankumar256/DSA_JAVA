...
public class anyBaseMultiplication {
	...
	public static int getProduct(int base, int num1, int num2) {
		...
		while (num2 > 0) {
			...
			int singleProduct = getSingleProduct(base, num1, digit2);

			result = getSum(base, result, singleProduct * power);
			power *= 10;
		}
		return result;
	}

	public static int getSingleProduct(int base, int num1, int digit2) {
		int result = 0;
		int carry = 0;
		int power = 1; // 10^0

		while (num1 > 0 || carry > 0) {
			int digit1 = num1 % 10; // taking out last digit for multiplication
			num1 /= 10; // removing last digit

			int product = digit1 * digit2 + carry;

			carry = product / base; // product = 12 , carry = 1 if base = 10
			product %= base; // total product = 12, then value = 2
